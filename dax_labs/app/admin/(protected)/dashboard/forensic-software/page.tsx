...
import { forensicSoftwareTools } from '@/app/data/forensic-software-tools';
import type { ForensicSoftwareData } from '@/app/data/forensic-software-tools';

const FILE_PATH = 'dax_labs/app/data/content/forensic-software-tools.json';

export default function EditForensicSoftware() {
  const [items, setItems] = useState<ForensicSoftwareData[]>(
    () => forensicSoftwareTools
  );
  ...
  function updateItem(index: number, field: keyof ForensicSoftwareData, value: unknown) {
    setItems((prev) => {
      const next = [...prev];
    ...
    <div className="min-h-screen bg-black text-white">
      <div className="max-w-5xl mx-auto px-6 py-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold">Forensic Software Tools</h1>
            <p className="text-gray-400 text-sm mt-1">{items.length} tools</p>
          </div>
          <button
            onClick={handleSave}
            disabled={saving}
